...
car.data.test$WheelTypeID <- as.factor(car.data.test$WheelTypeID)
...
car.data.test$IsBadBuyPred <- 0
car.data.test$IsBadBuyPred[car.data.test$IsBadBuyProb > 0.5] <- 1

#write.csv(car.data.test[c("RefId","IsBadBuy")], "NaiveModelPredictions.csv", row.names=FALSE)

confusionMatrix(table(car.data.test$IsBadBuy,car.data.test$IsBadBuyPred))
